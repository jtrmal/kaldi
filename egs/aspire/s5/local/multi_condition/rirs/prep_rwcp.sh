...
# so we scale the values in python3 first
...
for data_file in ${data_files[@]}; do
  ...
  python3 $tempdir_robo/raw_read.py $data_file $temp_file
...
type_num=5
noise_patterns=( $(ls ${output_dir}/RWCP_type${type_num}_noise*.wav | xargs -n1 basename | python3 -c"
import sys
