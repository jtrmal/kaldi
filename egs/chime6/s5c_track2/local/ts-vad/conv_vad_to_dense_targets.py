...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Usage: conv_vad_to_dense_targets.py <in-text-pos-vad-ali> <out-wspec> <utt2spk-1> <utt2spk-2> <utt2spk-3> <utt2spk-4> <segments-utt> <segments-ali> <utt2dur>"
    )
    ...
    with open(args.vad_ali) as f:
        for line in f:
            ...
            assert end - start >= len(vad_info), "{} {} {}".format(
                start, end, len(vad_info)
            )
            assert end - start - len(vad_info) <= 3, "{} {} {}".format(
                start, end, len(vad_info)
            )
            end = start + len(vad_info)
            seg_by_sess[sess].append(
                (utt_id_, spk, "", "", "", "", start, end, vad_info, device)
            )
    ...
    for uid in list(n_speakers.keys()):
        ...
        seg_by_sess[sess].append(
            (uid, spk, n_spk, n_spk2, n_spk3, n_spk4, start, end, [vad_info], "kinect")
        )
