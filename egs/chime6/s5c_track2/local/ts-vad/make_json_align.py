...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Usage: make_json_align.py <json-path> <ali-wspec>"
    )
    ...
    assert os.path.isfile(args.json_path), "File does not exist {}".format(
        args.json_path
    )
    ...
    for reco in data:
        ...
        num_frames = (
            int((end_time - start_time) * args.frequency) - frame_len
        ) // frame_shift + 1
