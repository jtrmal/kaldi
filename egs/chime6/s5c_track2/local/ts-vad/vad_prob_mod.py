    ...
    def __init__(self, vad_rspec, reg_exp):
        ...
        with ReadHelper(vad_rspec) as reader:
            for utid, align in reader:
                ...
                assert (
                    result is not None
                ), 'Wrong VAD alignment utterance ID format: "{}"'.format(utid)
                sess = result.group(1)
        ...
        for sess in self.data.keys():
            for piece in self.data[sess].keys():
                ...
                for spkr in self.data[sess][piece].keys():
                    for i in range(len(self.data[sess][piece][spkr])):
                        if (
                            self.data[sess][piece][spkr][i] / sumprob[i]
                            < multispk_threshold
                        ):
                            self.data[sess][piece][spkr][i] = lowest_value
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Usage: vad_prob_mod.py <vad-rspec> <vad-wspec>"
    )
    parser.add_argument(
        "--reg_exp", "-x", type=str, default=r"^(S\d\d.*)\-(\d+)\-(\d)$"
    )
    ...
    print(
        "  Multispeaker threshold for Pi/(P1+P2+P3+P4):     {}".format(
            args.multispk_threshold
        )
    )
    print(
        "  Lowest value which is used when applying the thresholds:    {}".format(
            args.lowest_value
        )
    )
