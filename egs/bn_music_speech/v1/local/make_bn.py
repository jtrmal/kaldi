...
with open(os.path.join(out_dir, "utt2spk"), "w") as utt2spk_fi, open(
    os.path.join(out_dir, "segments"), "w"
) as segments_fi, open(os.path.join(out_dir, "wav.scp"), "w") as wav_fi, open(
    os.path.join(out_dir, "utt_list"), "r"
) as utt_fi:
    ...
    for subdir, dirs, files in os.walk(wav_dir):
        for f in files:
            ...
            if f.endswith(".sph") and utt in utts:
                wav_fi.write(
                    "{0} sox {1}/{0}.sph -c 1 -r 16000 -t wav - |\n".format(utt, subdir)
                )

        for utt in utts:
            music_filename = utt + "_music.key.refined"
            speech_filename = utt + "_speech.key.refined"
            with open(os.path.join(out_dir, music_filename), "r") as music_fi, open(
                os.path.join(out_dir, speech_filename), "r"
            ) as speech_fi:
