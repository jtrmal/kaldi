...
if [ $# != 4 ]; then
  ...
  echo ""
  echo "Usage: align_text.sh <data-dir> <lang-dir> <local-lang-dir> <exp-dir>"
  echo " e.g.: align_text.sh data/heldout data/lang data/local/lang exp/heldout_ali"
...
if [ ! -f $lang/L_align.fst ]; then
  ...
  local/make_L_align.sh $locallang $lang $lang 2>&1 | tee $dir/log/L_align.log
fi
...
local/txt2rttm.pl --symtab=$lang/words.txt --segment=$data/segments $dir/align.txt $dir/rttm 2>$dir/log/rttm.log
