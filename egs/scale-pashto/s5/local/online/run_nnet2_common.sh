...
if [ $stage -le 1 ]; then
  ...
  for datadir in train dev10h; do
...
if [ $stage -le 3 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 64 --num-threads 8  --parallel-opts " --num-threads 8" --num-frames 700000 \
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 --num-threads 2 --parallel-opts " --num-threads 8" \
