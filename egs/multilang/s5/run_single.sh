...
# dictionary selection....  result in data/local/dict/filtered_lexicon.txt
...
if [[ ! -f data/local/dict/lexicon.txt ]]; then
  ...
  romanized=true
  if [ ! -z ${train_list_secondary+x}  ] && [ ! -z ${train_list_primary+x} ]  ; then
      echo "Merging the lexicons..." # exp_AB
      lex1=data/local/dict/filtered_primary_lexicon.txt
      lex2=data/local/dict/filtered_secondary_lexicon.txt
      
      cat data/local/dict/filtered_*_lexicon.txt | \
          local/lexicon_to_babel_format.pl > data/local/dict/filtered_lexicon.txt
      
  elif  [ ! -z ${train_list_secondary+x}  ] ; then
      echo "Using only the secondary Lexicon..."  # exp_B
      lex2=data/local/dict/filtered_secondary_lexicon.txt
      ...
      echo "Using only the primary Lexicon..."  # exp_A
      lex1=data/local/dict/filtered_primary_lexicon.txt
...
if [[ ! -f data/train_primary/wav.scp ]]; then 
  ...
  echo ---------------------------------------------------------------------
  echo "Please run run-0-data.sh first"
#  exit 1

fi

if [[ ! -f data/train_secondary/wav.scp  ]]; then
  echo ---------------------------------------------------------------------
  echo "Preparing acoustic training lists in data/train_secondary on" `date`
  echo ---------------------------------------------------------------------
  echo "Please run run-0-data.sh first"
#  exit 1
...
# based on config, combine directories
if [ ! -f data/train/.done ] ; then
  if [ ! -z ${train_list_primary+x}  ] && [ ! -z ${train_list_secondary+x} ]  ; then
    combine_data.sh data/train data/train_primary data/train_secondary
  elif  [ ! -z ${train_list_secondary+x}  ] ; then
    utils/copy_data_dir.sh data/train_secondary data/train
  else
    utils/copy_data_dir.sh data/train_primary data/train
  fi
