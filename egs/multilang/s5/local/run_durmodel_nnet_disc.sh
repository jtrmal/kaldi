...
if [ $stage -le 0 ]; then
  ...
  # The epoch3 has the best wer
  steps/nnet2/align.sh --nj $nj --cmd "$train_cmd" --use-gpu no --iter epoch3 \
      --transform-dir "$transform_dir" --online-ivector-dir exp/nnet2_online/ivectors_train_hires_sp2 \
      data/train_hires_sp data/langp_test exp/nnet2_online/nnet_ms_h_sp_smbr_0.000005  exp/nnet2_online/nnet_ms_h_sp_smbr_0.000005_ali/ || exit 1
fi
...
if [ $stage -le 1 ]; then
  ...
  ./dur-model/train_dur_model.sh --nj $nj --cmd "$cuda_cmd --gpu 0" --cuda-cmd "$cuda_cmd --mem 8g" --pylearn-dir $pylearn_dir --aggregate-data-args "$aggregate_data_args" \
    ...
    data/train_hires_sp data/langp_test exp/nnet2_online/nnet_ms_h_sp_smbr_0.000005_ali exp/dur_model_nnet_ms_h_sp_smbr || exit 1
fi
...
for epoch in 2 3 4; do
  ...
  for decode_set in  dev10h dev_appen; do
	  ...
	  ./dur-model/decode_dur_model.sh --cmd "$train_cmd" --cuda-cmd "$cuda_cmd --mem 8g" --nj $num_jobs \
		...
		--scales "0.2 0.3 0.4" --penalties "0.21 0.23 0.25 0.28 0.30 0.33" \
		--stage 0 --iter epoch3 \
		--left-context $left_context --right-context $right_context \
		data/langp_test \
		exp/nnet2_online/nnet_ms_h_sp/graph/ \
		data/${decode_set}_hires \
		exp/nnet2_online/nnet_ms_h_sp_smbr_0.000005/decode_epoch${epoch}_${decode_set} \
		exp/dur_model_nnet_ms_h_sp_smbr \
		exp/nnet2_online/nnet_ms_h_sp_smbr_0.000005/decode_epoch${epoch}_${decode_set}.dur-rescore || exit 1;
	done
  ) &
done
fi
