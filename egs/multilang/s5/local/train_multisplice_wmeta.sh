...
if [ ! -z $metadata_feats ]; then
 meta_dim=`head -1 $metadata_feats | wc -w`; # minus 3
 meta_dim=$[ $meta_dim - 3 ]
 metadata_args="--metadata-feats $metadata_feats"
...
if [ $stage -le -4 ]; then
  ...
  local/get_lda_meta.sh $lda_opts "${extra_opts[@]}" $metadata_args --left-context $first_left_context --right-context $first_right_context --cmd "$cmd" $data $lang $alidir $dir || exit 1;
fi
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  local/get_nnet_egs.sh $egs_opts "${extra_opts[@]}" \
...
if [ $stage -le -2 ]; then
  ...
  lda_mat=$dir/lda.mat
  echo "Dims iv = $ivector_dim meta $meta_dim"
  ivector_dim=$[ $ivector_dim + $meta_dim ]
...
if [ $stage -le -1 ]; then
  ...
  echo $train_cmd $dir/log/train_trans.log \
