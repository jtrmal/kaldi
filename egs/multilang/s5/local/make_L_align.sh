...
[ ! -d $tmpdir ] && echo "Directory $tmpdir does not exist! " && exit 1
[ ! -d $dir ] && echo  "Directory $dir does not exist! " && exit 1
[ ! -d $outdir ] && echo "Directory $outdir does not exist! " && exit 1

# Create lexicon with alignment info
if  [ -f $tmpdir/lexiconp.txt ] ; then
  cat $tmpdir/lexiconp.txt | \
    awk '{printf("%s #1 ", $1); for (n=3; n <= NF; n++) { printf("%s ", $n); } print "#2"; }'
elif [ -f $tmpdir/lexicon.txt ] ;  then
