...
def get_args():
    ...
    parser.add_argument(
        "feature_scp", help="Input gop-based feature file, in Kaldi scp"
    )
    parser.add_argument(
        "--phone-symbol-table", type=str, default="", help="Phone symbol table"
    )
    parser.add_argument(
        "human_scoring_json", help="Input human scores file, in JSON format"
    )
    parser.add_argument("output", help="Output the picture")
    parser.add_argument(
        "--samples", type=int, default=500, help="The number of the examples to draw"
    )
...
def main():
    ...
    # Sampling
    sampled_paris = random.sample(
        list(zip(features, lables)), min(args.samples, len(lables))
    )
    ...
    sns_plot = sns.scatterplot(
        x=features[:, 0], y=features[:, 1], hue=lables, legend="full", palette=colors
    )
