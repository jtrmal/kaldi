...
cat $srcdir/utt2spk | apply_map.pl -f 1 $destdir/utt_map  | \
  apply_map.pl -f 2 $destdir/spk_map >$destdir/utt2spk

utt2spk_to_spk2utt.pl <$destdir/utt2spk >$destdir/spk2utt

if [ -f $srcdir/feats.scp ]; then
  apply_map.pl -f 1 $destdir/utt_map <$srcdir/feats.scp >$destdir/feats.scp
fi


if [ -f $srcdir/segments ]; then
  apply_map.pl -f 1 $destdir/utt_map <$srcdir/segments >$destdir/segments
  ...
  if [ -f $srcdir/wav.scp ]; then 
    apply_map.pl -f 1 $destdir/utt_map <$srcdir/wav.scp >$destdir/wav.scp
  fi
fi

if [ -f $srcdir/text ]; then
  apply_map.pl -f 1 $destdir/utt_map <$srcdir/text >$destdir/text
fi
if [ -f $srcdir/spk2gender ]; then
  apply_map.pl -f 1 $destdir/spk_map <$srcdir/spk2gender >$destdir/spk2gender
fi
if [ -f $srcdir/cmvn.scp ]; then
  apply_map.pl -f 1 $destdir/spk_map <$srcdir/cmvn.scp >$destdir/cmvn.scp
fi
...
validate_data_dir.sh $validate_opts $destdir
