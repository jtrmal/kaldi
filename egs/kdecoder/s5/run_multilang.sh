...
if [ $stage -le 0 ]; then
  steps/nnet2/train_multilang2.sh --parallel-opts "$parallel_opts" \
    ...
    --num-epochs 6 --mix-up "0 0 0 0" --num-jobs-nnet "6 4 4 4"\
    --cmd "$train_cmd --config conf/slurm.conf"  --stage $train_stage \
    ./exp/tri3_ali_sp  ./exp/nnet2_online/nnet_ms_h_sp/egs\
    ../exp_10_A/exp/tri3_ali_sp ../exp_10_A/exp/nnet2_online/nnet_ms_h_sp/egs\
    ../exp_10_B/exp/tri3_ali_sp ../exp_10_B/exp/nnet2_online/nnet_ms_h_sp/egs\
    ../exp_10_T/exp/tri3_ali_sp ../exp_10_T/exp/nnet2_online/nnet_ms_h_sp/egs\
...
if [ $stage -le 1 ] ; then
  ...
  utils/mkgraph.sh  data/langp_test_T/ exp_multilang/$system/3 \
...
if [ $stage -le 2 ]; then
  ...
  steps/online/nnet2/prepare_online_decoding.sh --mfcc-config conf/mfcc_hires.conf\
    data/langp_test_A/ ../exp_10_A/exp/nnet2_online/extractor \
    exp_multilang/${system}/1/ \
    exp_multilang/${system}_online/1/

  mkdir -p exp_multilang/${system}_online/2/
  steps/online/nnet2/prepare_online_decoding.sh --mfcc-config conf/mfcc_hires.conf\
    data/langp_test_B/ ../exp_10_B/exp/nnet2_online/extractor \
    exp_multilang/${system}/2/ \
    exp_multilang/${system}_online/2/

  mkdir -p exp_multilang/${system}_online/3/
  steps/online/nnet2/prepare_online_decoding.sh --mfcc-config conf/mfcc_hires.conf\
    data/langp_test_T/ ../exp_10_T/exp/nnet2_online/extractor \
    exp_multilang/${system}/3/ \
    exp_multilang/${system}_online/3/
fi
