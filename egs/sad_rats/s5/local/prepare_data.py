...
def write_output(segments, out_path, min_length):
    reco_and_spk_to_segs = defaultdict(
        list,
        ...
        for uid in sorted(reco_and_spk_to_segs):
            ...
            for seg in segs:
                ...
                if seg.sad_label == "NS":
                    ...
                    rttm_writer.write(
                        rttm_str.format(reco_id, seg.start_time, seg.dur, spk_id)
                    )
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "wav_path", help="Path to source rats_sad corpus audio files directory"
    )
    ...
    parser.add_argument(
        ...
        default=0.0001,
        type=float,
        help="minimum length of segments to create",
    )
