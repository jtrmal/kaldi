    ...
    for line in content:
        ...
        for i in range(start_time, end_time):
            if i < 0:
                ...
                print(
                    "Time index exceeds number of frames. Cropping RTTM from {} to {} \
                      frames".format(
                        end_time, num_frames
                    )
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Frame-level overlap reassignment with speaker posterior attributions"
    )
    ...
    parser.add_argument(
        "init_rttm", type=str, help="Initial RTTM file (to get voiced segments)"
    )
    parser.add_argument(
        "ovl_dir", type=str, help="Working directory for VB resegmentation"
    )
    ...
    for utt in utt_list:
        ...
        mask = vad >= 2
        print(
            "Recording: {}, Num frames: {}, voiced: {}".format(
                utt, n_frames, mask.sum()
            )
        )
