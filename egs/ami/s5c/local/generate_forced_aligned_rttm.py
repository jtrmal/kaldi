...
def main():
    ...
    reco_and_spk_to_words = defaultdict(
        list,
        {
            reco_id: list(g)
            for reco_id, g in groupby(ctm_words, lambda x: (x.reco_id, x.spk_id))
        },
    )
    ...
    for segment in sorted(new_segments, key=lambda x: (x[0], x[1])):
        print(
            rttm_str.format(segment[0], segment[1], segment[2] - segment[1], segment[3])
        )
