...
roman_number = [
    re.compile(r"^\s*[_LXVI]+(\.)?\s*$"),
]

sq_brackets = [re.compile(r"(.*)(\[.+\])(.*)", re.IGNORECASE)]
...
chapter = [
    re.compile(r"^\s*((Chapter)|(Volume)|(Canto)).*[LXIV0-9]+.*$", re.IGNORECASE),
]

contents = [
    re.compile(r"CONTENTS"),
    re.compile(r"^.*((\s{2,50})|([\t]+))[0-9]+\s*$"),
    re.compile(r"^\s*((I+[:.]+)|(I?[LXV]+I*([\.:])?))\s+.*"),
]
