...
for b in $(cat $in_list); do
  ...
  mkdir -p $out_root/$id
  python3 local/lm/python3/pre_filter.py $in_file /dev/stdout |\
    python3 local/lm/python3/text_pre_process.py /dev/stdin /dev/stdout |\
    nsw_expand -format opl /dev/stdin |\
    python3 local/lm/python3/text_post_process.py /dev/stdin $out_file /dev/null || exit 1
