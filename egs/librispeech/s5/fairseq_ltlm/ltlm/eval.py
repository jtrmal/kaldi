...
def compute_model_wer(
    ...
    **kwargs,
    ...
    logger.info(
        f"Compute wer. acwt={acwt}. lmwt={lmwt}. hyp_filter={hyp_filter}. kwargs={kwargs}"
    )
    ...
    if len(removed_lats) > 0:
        ...
        for lat, weights, utt in zip(removed_lats, removed_lats_ws, removed_lats_utts):
            ...
            assert hyp_line[0] == "<s>" and hyp_line[-1] == "</s>", RuntimeError(
                f"{hyp_line}"
            )
            utt2hyp[utt] = hyp_line[1:-1]
    wer_str = compute_wer(ref_fname, utt2hyp, keep_tmp=keep_tmp, hyp_filter=hyp_filter)[
        0
    ]  # 0 - wer, 1 - ser
...
def get_scores(
    model, dataset, device="cpu", btz=1, progress_bar=True, dataloader_nj=12
):
    if model is not None:
    ...
    with torch.no_grad():
        for i, (batched_samples) in enumerate(iterator):
            ...
            for utt, utt_prob, words_ids in zip(
                batched_samples["utt_id"], probs_np, x[:, :, 0]
            ):
...
def apply_strategy(lat, lt_probs, strategy):
    assert strategy in RESCORE_STRATEGIES, RuntimeError(
        f"Bad rescoring strategy {strategy}. "
...
def get_rescoring_hyps(
    model, dataset, acwt=1, lmwt=1, model_weight=1.3, strategy="base", **kwargs
):
    assert strategy in RESCORE_STRATEGIES, RuntimeError(
        f"Bad rescoring strategy {strategy}. "
        f"Strategy must be in '{RESCORE_STRATEGIES}'"
    ...
    for utt, lt_probs in list(utt2score.items()):
        ...
        _, hyp = best_path_nloglike(lat, nll, final_word_id=final_word_id)
        hyp_line = tokenizer.decode([[arc[0] for arc in hyp]])[0]
        assert hyp_line[0] == "<s>" and hyp_line[-1] == "</s>", RuntimeError(
            f"{i} {hyp_line}"
        )
...
if __name__ == "__main__":
    ...
    rescoring_task.RescoringTask.add_args(
        parser, add_def_opts=False, add_data_opts=False
    )
    ...
    print(
        f"Lattices {dataset.get_name()}, model {args.model}, model_weight {args.model_weight}."
    )
    print(wer)
