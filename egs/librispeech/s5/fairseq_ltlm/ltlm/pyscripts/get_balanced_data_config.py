...
def balance_decode_gen_egs(
    decode_dirs, decode_suffs, gen_dirs, gen_suffs, epoch_max_MB=4000, reuse_data=True
):
    # Generate list [epoch] -> [(egs, suf), (egs,suff), ...]
    d2suff = {
        d: s
        for d, s in (
            *list(zip(decode_dirs, decode_suffs)),
            *list(zip(gen_dirs, gen_suffs)),
        )
    }
    ...
    if len(decode_f2s) > 0:
        logger.info(
            f"Found {len(decode_f2s)} egs with decoded lats. "
    ...
    if len(gen_f2s) > 0:
        logger.info(
            f"Found {len(gen_f2s)} egs with generated from text lats. "
    ...
    sorted_dec_f = [
        k for k, _ in sorted(list(decode_f2s.items()), key=lambda x: x[1], reverse=True)
    ]
    sorted_gen_f = [
        k for k, _ in sorted(list(gen_f2s.items()), key=lambda x: x[1], reverse=True)
    ]
    ...
    while (not reused_dec and dec_not_used.any()) or (
        ...
        if not insert:
            ...
            if len(curr_files) > 0:
                logger.info(
                    f"For epoch {len(out)} balance decode/generate is {round(curr_dec_sz/curr_gen_sz,6)}."
                )
                ...
                bad_dec = {
                    f: decode_f2s[f]
                    for f in map(sorted_dec_f.__getitem__, np.where(dec_not_used)[0])
                }
                bad_dec_str = "\n".join((f"{f} {s}" for f, s in list(bad_dec.items())))
                bad_gen = {
                    f: gen_f2s[f]
                    for f in map(sorted_gen_f.__getitem__, np.where(gen_not_used)[0])
                }
                bad_gen_str = "\n".join((f"{f} {s}" for f, s in list(bad_gen.items())))
                logger.warning(
                    f"This files is too big.\n DECODED LATS:\n"
                    f"{bad_dec_str}.\n "
                    f"GENERATE:\n {bad_gen_str}."
                )
                logger.warning(
                    "Careful!!! These files are inserted into training as 1 dump per data-epoch"
                )
                out.extend(
                    [
                        [(f, d2suff[os.path.dirname(f)])]
                        for f in (*list(bad_dec.keys()), *list(bad_gen.keys()))
                    ]
                )
                break
...
def main():
    ...
    parser.add_argument(
        "--valid", nargs="+", required=True, help="Valid. format 'egs,ref egs,ref2'"
    )
    parser.add_argument(
        "--test", nargs="*", default=None, help="Test. format like valid"
    )
    parser.add_argument(
        "--out", type=str, required=True, help="Path to result data_config.json file"
    )
    ...
    for l_r in args.valid:
        l_r_sep = l_r.split(",")
        assert len(l_r_sep) == 2, RuntimeError(
            "Wrong valid format. Must be lat,ref lat2,ref ..."
        )
        l, r = l_r_sep
    ...
    if args.test:
        ...
        for l_r in args.test:
            l_r_sep = l_r.split(",")
            assert len(l_r_sep) == 2, RuntimeError(
                "Wrong test format. Must be lat,ref lat2,ref ..."
            )
            l, r = l_r_sep
    ...
    epoch2data = balance_decode_gen_egs(
        train_dec_dirs,
        train_dec_suffs,
        train_gen_dirs,
        train_gen_suffs,
    ...
    data["train"] = [
        {"epoch": e, "lats": l, "utt_suff": s}
        for e, epoch_data in enumerate(epoch2data)
        for l, s in epoch_data
    ]
