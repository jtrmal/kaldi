...
def draw_rescoring_lats(
    ...
    with torch.no_grad():
        for i, (utt_id) in enumerate(iterator):
            ...
            x, weights = (
                batched_samples["net_input"]["src_tokens"],
                batched_samples["weights"],
            )
...
def main():
    ...
    parser.add_argument(
        "--format", type=str, default="pdf", help="Render file type. Default .pdf"
    )
    parser.add_argument(
        "--norm", action="store_true", default=False, help="Normalize lt probs"
    )
