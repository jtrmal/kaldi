...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hist_dir", type=str, default=None, help="Directory for saving histograms"
    )
    parser.add_argument(
        "--hist_bins", type=int, default=50, help="Histograms number of beam"
    )
    ...
    parser.add_argument(
        "--compare_clipped", action="store_true", help="Compare only clipped lattices"
    )
    ...
    report = d1.compare(
        ...
        normalize=False,
        compare_clipped=args.compare_clipped,
    )
    for k, v in list(report.items()):
        ...
        if args.hist_dir is not None:
            plot_hist(
                v,
                os.path.join(args.hist_dir, k.replace(" ", "_") + ".png"),
                bins=args.hist_bins,
                title=k,
            )
        ...
        print(
            f"{k} = {round(sum(v)/len(v), args.round)}. abs min = {v[min_i]} ({report['utts'][min_i]}). abs max = {v[max_i]} ({report['utts'][max_i]})."
        )
