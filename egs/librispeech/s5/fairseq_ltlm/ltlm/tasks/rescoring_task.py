...
class RescoringTask(FairseqTask):
    @staticmethod
    def add_args(
        parser: argparse.ArgumentParser, add_def_opts=True, add_data_opts=True
    ):
        ...
        if add_data_opts:
            parser.add_argument(
                ...
                type=str,
                required=True,
                help="Data egs config in json format",
            )
        ...
        parser.add_argument(
            "--infer_btz", type=int, default=8, help="Batch size for evaluating model"
        )
        parser.add_argument(
            "--hyp_filter",
            type=str,
    ...
    def load_data_config(self):
        ...
        self.val_test_conf = {
            "valid": self.data_config.get("valid", []),
    ...
    def load_dataset(self, split, combine=False, epoch=0, **kwargs):
        ...
        if split in list(self.datasets.keys()):
            ...
            return
        if split == "train":
            self.datasets[split] = PerEpochWrapper(
                self.cfg, self.tokenizer, self.train_conf, dataset_cls=self.data_cls
            )
            return

        for set_data in self.val_test_conf[split]:
            lats_data, ref_text = set_data["lats"], set_data["ref"]
            assert ref_text is not None, RuntimeError(
                f"ref_text for {split}:({lats_data}) required!"
            )
            ds = self.data_cls.build_from_kwargs(
                lats_data=lats_data,
            ...
            logger.info(
                f"For split {split}:({lats_data}) ORACLE WER is {round(ds.oracle_wer()[0], 2)}"
            )
            ...
            logger.info(
                f"For split {split}:({lats_data}) wer without rescoring is {wer}"
            )
