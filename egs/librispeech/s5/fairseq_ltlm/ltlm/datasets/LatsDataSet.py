...
from ltlm.pyutils.lattice_utils import (
    topsort_lat,
    oracle_path,
    padding,
    parse_lats,
...
class LatsDataSet(FairseqDataset):
    ...
    def build_kwargs(cls, args, tokenizer, prefix=""):
        ...
        return {
            "lats_data": getattr(args, f"{prefix}data"),
    ...
    def get_data_from_disc(self, data_dict_list, data_type="dump"):
        assert data_type in ["lat_t", "dump"], RuntimeError(
            f"Wrong data type {data_type}"
        )
        ...
        for data_dict in data_dict_list:
            ...
            if data_type == "lat_t":
                if curr_data == "-":
                    ...
                    self.add_data_from_latt(
                        sys.stdin.readlines(), recompute_utt2id=False, suff=suff
                    )
                else:
                    if os.path.isdir(curr_data):
                        logger.info(
                            f"{curr_data} is directory. Loading {curr_data}/lat.*.t"
                        )
                    ...
                    for fn in glob(curr_data):
                        ...
                        with open(fn, "r", encoding="utf-8") as f:
                            self.add_data_from_latt(
                                f.readlines(), recompute_utt2id=False, suff=suff
                            )
            elif data_type == "dump":
                if os.path.isdir(curr_data):
                    logger.info(
                        f"{curr_data} is directory. Loading {curr_data}/lat.*.dump"
                    )
    ...
    def add_data_from_dicts(self, data_dicts, recompute_utt2id=True, suffs=[""]):
        for i, (d, s) in enumerate(zip(data_dicts, suffs)):
            self.add_list_data_from_dict(
                d,
                recompute_utt2id=(recompute_utt2id and i == len(data_dicts) - 1),
                suff=s,
            )
    ...
    def load_ref(self, ref_text_fname):
        ...
        with open(ref_text_fname, "r", encoding="utf-8") as f:
            utt2ref = {
                utt_id: self.tokenizer.encode([line], bos=True, eos=True)[0]
                for utt_id, line in [
                    x.split(" ", 1) if x.find(" ") != -1 else [x.strip(), ""] for x in f
                ]
                if utt_id in utts
            }
        logger.info(f"LatsDataSet: loaded {len(utt2ref)} ref.")
        assert len(utt2ref) == len(
            self.id2utt
        ), f"Utterance name {set(self.id2utt) - set(utt2ref.keys())} not found in file {ref_text_fname}"
        return utt2ref

    def cliped_data(self, max_len):
        # Clip max len
        self.max_len = max_len
        self.clipid2len_id = sorted(
            [(len(lat), i) for i, lat in enumerate(self.id2lat) if len(lat) < max_len]
        )
        self.too_big_lats = [
            i for i, lat in enumerate(self.id2lat) if len(lat) >= max_len
        ]
        num_clipped = len(self.id2lat) - len(self.clipid2len_id)
        logger.info(
            f"LatsDataSet: Clipping with max_len={max_len} remove {num_clipped} utts "
    ...
    def __getitem__(self, item):
        ...
        return {
            "net_input": {
                "src_tokens": torch.LongTensor(lat),
            },
            "weights": weights,
            "utt_id": utt_id,
    ...
    def compact_lat_to_lat(self, compact_lat):
        ...
        for arc in compact_lat:
            if len(arc) == 4:
                ...
                new_state_from, new_state_to = (
                    state_from + new_id_shift,
                    state_to + new_id_shift,
                )
                ...
                raise RuntimeError(
                    "unknown arc len in compact_lat_to_lat. Need to debug"
                )
        # Add EOS symbols
    ...
    def get_compact_lattices(self):
        comp_lats = {}
        new_id_shift = 2
        for i, (utt_id, lat, weights, ali) in enumerate(
            zip(self.id2utt, self.id2lat, self.id2weights, self.id2p_ali)
        ):
            comp_lat = []
            for (word_id, state_from, state_to), (w_hcl, w_ac), p_ali in zip(
                lat, weights, ali
            ):
                state_from, state_to = (
                    state_from - new_id_shift,
    ...
    def compare(self, other, normalize=True, progress_bar=True, compare_clipped=False):
        ...
        for i in iterator:
            ...
            delta_bags = sum(counters_intersection.values()) / sum(
                counters_union.values()
            )
        ...
        return {
            ...
            "Average bag of uniq words correlation": sum(bag_uniq_stats)
            / len(bag_uniq_stats),
            "Average different between num arcs in first and second lat": sum(
                num_arcs_compare
            )
            / len(num_arcs_compare),
            "Average absolute different between num arcs": sum(
                map(abs, num_arcs_compare)
            )
            / len(num_arcs_compare),
            "composition_stat": 0,
        }
    ...
    def oracle_wer(self, ref_text_fname, print_interval=5000):
        ...
        with open(ref_text_fname, "r", encoding="utf-8") as f:
            utt2ref = {
                utt_id: self.tokenizer.encode([line], bos=True, eos=True)[0]
                for utt_id, line in [
                    x.split(" ", 1) if x.find(" ") != -1 else [x.strip(), ""]
        ...
        for i, (utt_id, ref) in enumerate(utt2ref.items()):
            ...
            err, ali = oracle_path(
                lat,
                ref,
                final_word_id=self.tokenizer.get_eos_id(),
            ...
            if i % print_interval == 0:
                logging.info(
                    f"Oracle wer processed {round(i / total_count * 100, 2)}%. "
                )
