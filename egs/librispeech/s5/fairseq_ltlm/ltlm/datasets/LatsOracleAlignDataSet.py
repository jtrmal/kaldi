...
class LatsOracleAlignDataSet(LatsDataSet):
    ...
    def build_from_kwargs(cls, **kwargs):
        ...
        obj = cls(
            ...
            ref_text_fname=kwargs["ref_text_fname"],
            all_oracle_targets=kwargs["all_oracle_targets"],
        )
        obj.get_data_from_disc(
            parse_lats_data_str(kwargs["lats_data"]), kwargs["data_type"]
        )
        if kwargs["data_type"] == "lat_t":
            assert kwargs["ref_text_fname"] is not None, RuntimeError(
                f"For data type lat_t --ref_text_fname required!"
            )
            assert os.path.exists(kwargs["ref_text_fname"]), RuntimeError(
                f"{kwargs['ref_text_fname']} not exist!"
            )
            obj.load_ref(kwargs["ref_text_fname"])
            obj.compute_oracle_ali()
            logger.info(
                f"Oracle wer is {round(obj.oracle_err_sum/obj.num_ref_words*100,2)}%"
            )
        if kwargs["clip"]:
    ...
    def add_data_from_dicts(self, data_dicts, recompute_utt2id=True, suffs=[""]):
        ...
        self.utt2ref = {
            k + suff: v
            for data_dict, suff in zip(
                [{"utt2ref": self.utt2ref}] + data_dicts, [""] + suffs
            )
        ...
        self.utt2ohyp = {
            k + suff: v
            for data_dict, suff in zip(
                [{"utt2ohyp": self.utt2ohyp}] + data_dicts, [""] + suffs
            )
        ...
        self.utt2ali = {
            k + suff: v
            for data_dict, suff in zip(
                [{"utt2ali": self.utt2ali}] + data_dicts, [""] + suffs
            )
    ...
    def cliped_data(self, max_len, clip_one_path=True):
        ...
        if clip_one_path:
            self.clipid2len_id = [
                (l, i)
                for l, i in self.clipid2len_id
                if len(self.utt2ali[self.id2utt[i]]) < len(self.id2lat[i])
            ]
            all_true_lats = [
                i
                for l, i in self.orig_clipid2len_id
                if len(self.utt2ali[self.id2utt[i]]) >= len(self.id2lat[i])
            ]
            num_clipped = len(all_true_lats)
            self.too_big_lats.extend(all_true_lats)
            logger.info(
                f"LatsOracleAlignDataSet: Clipping also remove {num_clipped} utts with only oracle paths."
    ...
    def compute_oracle_ali(self, print_interval=100):
        ...
        for i, (utt_id, ref) in enumerate(self.utt2ref.items()):
            ...
            if len(lat) > self.max_len:
                logger.info(
                    f"Skip {utt_id}. len ({len(lat)}) > max len ({self.max_len})"
                )
            # logger.debug(f"Process {utt_id}")
            err, oracle_arcs = oracle_path(
                lat,
                ref,
            ...
            if (i + 1) % print_interval == 0:
                logger.info(
                    f"Oracle wer processed {i} utts ({round(i/total_count*100, 2)}%)."
                )
        self.oracle_err_sum, self.num_ref_words, self.utt2ohyp, self.utt2ali = (
            global_err,
            global_len,
            hyps,
            alis,
    ...
    def oracle_wer(self, *args, **kwargs):
        return (
            self.oracle_err_sum / self.num_ref_words * 100,
            self.utt2ohyp,
            self.utt2ali,
        )
    ...
    def get_statistic(self):
        ...
        stats["Target arcs"] = sum([len(a) for a in list(self.utt2ali.values())]) / len(
            self.id2utt
        )
        stats["Non-target arcs"] = sum(
            [
                len(self.id2lat[i]) - len(self.utt2ali[utt])
                for i, utt in enumerate(self.id2utt)
            ]
        ) / len(self.id2utt)
        return stats
