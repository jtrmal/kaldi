...
class WordTokenizerException(Exception):
    pass
...
class WordTokenizer:
    ...
    def __init__(self, fname, unk_word="<UNK>"):
        ...
        if self.unk not in self.word2id:
            if self.unk.lower() in self.word2id:
                ...
                raise WordTokenizerException(f"unk word {unk_word} not in {fname}")

        for w, i in list(self.word2id.items()):
            self.id2word[i] = w
        assert (
            self.word2id["<eps>"] == 0
            and "<s>" in list(self.word2id.keys())
            and "</s>" in list(self.word2id.keys())
        ), RuntimeError("<esp>!=0")
        ...
        logger.info(
            f"WordTokenizer Disambig words: {[ self.id2word[i] for i in self.disambig_word_ids]}"
        )
    ...
    def encode(self, text, bos=False, eos=False):
        return [
            ...
            + [
                self.word2id[w]
                if w in list(self.word2id.keys())
                else self.word2id[self.unk]
                for w in line.split()
            ]
            + ([self.get_eos_id()] if eos else [])
            for line in text
        ]
    ...
    def print_lat(self, lat, print_word_id=False, p=None):
        for i, arc in enumerate(lat):
            out_str = (
                f"{arc[1]} {arc[2]} {arc[0] if print_word_id else self.id2word[arc[0]]}"
            )
            if p is not None:
                out_str += f" {p[i]}"
            print(out_str)
