...
class AliStretchModel:
    ...
    def build_kwargs(args):
        kwargs = {"model_path": args.stretch_model_path, "max_pdf_id": args.max_pdf_id}
        return kwargs
    ...
    def compute(self):
        ...
        if np.any(self.id2count == 0):
            ...
            logger.warning(
                f"Not all pdf ids found in train data. bad pdf_ids = {bad_ids}. shape={bad_ids.shape}"
    ...
    def forward(self, ids):
        # ids - [0, 1, 0,...]
        dup_ids = np.concatenate(
            [
                np.array([index] * self.sample_seq_len(index), dtype=np.int32)
                for index in ids
            ]
        )
        return dup_ids
    ...
    def sample_seq_len(self, index):
        ...
        if len(distr) == 0:
            return 1
        population, weights = np.asarray(list(distr.keys())), np.asarray(
            list(distr.values())
        )
...
class Id2LoglikeAMModel:
    ...
    def build_kwargs(args):
        kwargs = {
            "model_path": args.id2ll_model_path,
    ...
    def add_utts(self, ids, probs):
        assert len(ids.shape) == 1 and len(probs.shape) == 2, RuntimeError(
            f"Wrong shape in add_probs"
        )
        assert ids.shape[0] == probs.shape[0], RuntimeError(
            f"Ali ({ids.shape[0]}) and features ({probs.shape[0]}) len not the same!"
        )
...
if __name__ == "__main__":
    ...
    utt2ali = {
        key: ali
        for key, ali in tqdm(
            kaldi_io.read_vec_int_ark(f"ark: gunzip -c {args.ali_dir}/ali_pdf.1.gz|")
        )
    }
    i = 0
