...
BaseFloat GenericNumeratorComputation::AlphaGeneralFrame(int32 seq) {
  ...
  CopySpecificPdfsIndirect(seq, index_to_pdf_[seq], &(probs_[seq]));
  ...
  for (int t = 1; t <= num_frames; ++t) {
    SubMatrix<BaseFloat> prev_alpha_t(alpha, t - 1, 1, 0,
                                      alpha_[seq].NumCols() - 1);
    SubMatrix<BaseFloat> this_alpha_t(alpha, t, 1, 0,
                                      alpha_[seq].NumCols() - 1);
    ...
    double sum = alpha(t-1, alpha.NumCols() - 1);
...
bool GenericNumeratorComputation::Backward(
  ...
  for (int seq = 0; seq < num_sequences; ++seq) {
    ...
    BetaGeneralFrameDebug(seq);
    AddSpecificPdfsIndirect(seq, nnet_output_deriv_[seq],
...
void GenericNumeratorComputation::BetaGeneralFrame(int32 seq) {
  ...
  for (int t = num_frames - 1; t >= 0; --t) {
    ...
    const SubVector<BaseFloat> next_beta(beta_[seq].RowData((t + 1) % 2),
                                         num_states);
...
void GenericNumeratorComputation::BetaGeneralFrameDebug(int32 seq) {
